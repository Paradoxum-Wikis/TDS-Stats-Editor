    ...
    getTowerCostForLevel(towerName, level) {
        ...
        try {
            ...
            if (level > 0 && towerInfo.Default.Upgrades) {
                for (let i = 0; i < level && i < towerInfo.Default.Upgrades.length; i++) {
                    // make sure the upgrade has a cost defined
                    const upgradeCost = towerInfo.Default.Upgrades[i]?.Cost || 0; 
                    price += upgradeCost;
                    TowerRegistry.log(`registry after adding level ${i+1} cost (${upgradeCost}): ${price}`);
                }
            }
            
            TowerRegistry.log(`registry final price for ${towerName} level ${level}: ${price}`);
            return price;
        } catch (e) {
