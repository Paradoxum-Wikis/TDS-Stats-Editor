...
import { renderMapGallery, renderTowerGallery, renderPreselectedTowers, renderExcludedMaps, renderExcludedTowers } from './components/Render.js';
...
import { setupMobileNav, populateControlsSection, isSidebarOpen, closeSidebar } from './components/MobileNav.js';
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    ...
    if (appState.allMaps.length === 0 || appState.allTowers.length === 0 || appState.allModes.length === 0) {
      showError(UI_ELEMENTS.randomizedResultsContainer, "Some data could not be loaded. Please check console for details.");
    } else {
      UI_ELEMENTS.randomizedResultsContainer.innerHTML = '<p class="text-muted text-center mb-0">Click "Randomize!" to generate your challenge!</p>';
    }
    ...
    showError(UI_ELEMENTS.randomizedResultsContainer, "Error fetching initial data.");
  ...
  UI_ELEMENTS.copyResultBtn.addEventListener('click', async () => {
    if (!appState.lastSelectedMap && !appState.lastSelectedMode && appState.lastFinalTowers.length === 0) {
