...
export function renderRandomizedResult(selectedMap, selectedMode, selectedTowers) {
...
export function renderMapGallery(maps) {
  if (maps.length === 0) {
    UI_ELEMENTS.mapGalleryContainer.innerHTML = '<p class="text-muted text-center mb-0">No maps available.</p>';
    return;
  }
...
export function renderTowerGallery(towers) {
  if (towers.length === 0) {
    UI_ELEMENTS.towerGalleryContainer.innerHTML = '<p class="text-muted text-center mb-0">No towers available.</p>';
    return;
  }
