export class BuildManager {
  ...
  static loadBuild(buildDataString) {
    ...
    try {
      ...
      return { success: false, error: 'Invalid build data format: ' + error.message };
    }
  }
  ...
  static copyURLToClipboard(url) {
    navigator.clipboard.writeText(url).then(() => {
      alert('Build URL copied to clipboard! Share this link with others to show your skill build.');
    }).catch(() => {
      // older browsers
