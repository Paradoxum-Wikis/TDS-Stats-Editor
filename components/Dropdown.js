class Dropdown {
    ...
    switch (event.key) {
      ...
      case "Enter":
        if (
          ...
          this.currentHighlightedIndex < options.length
        ) {
          event.preventDefault();
          options[this.currentHighlightedIndex].dispatchEvent(
  ...
  #navigateDropdown(direction, options) {
    ...
    if (
      ...
      options[this.currentHighlightedIndex].classList.add(
        "keyboard-highlighted",
      );

      options[this.currentHighlightedIndex].scrollIntoView({
        block: "nearest",
      });
    }
  }
  ...
  #removeHighlightFromAll() {
    const highlightedItems = this.dropdown.querySelectorAll(
      ".keyboard-highlighted",
    );
