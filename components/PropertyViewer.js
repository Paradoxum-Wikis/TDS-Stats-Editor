...
export default class PropertyViewer {
  constructor(viewer, root) {
    ...
    this.hidden = ["NoTable", "SideLevel", "Abilities.0", "Abilities", "Note"];
    ...
    this.towerPropsBtn.addEventListener(
      "click",
      this.showTowerProperties.bind(this),
    );
    ...
    this.unitPropsBtn.addEventListener(
      "click",
      this.showUnitProperties.bind(this),
    );
...
//  }
  ...
  isHidden(property) {
    ...
    if (this.currentView === "tower") {
      ...
      if (
        (this.isMilitaryBaseTower() ||
          this.isMechaBaseTower() ||
          this.isElfCampTower()) &&
        (property === "Damage" ||
          property === "Cooldown" ||
          property === "Range" ||
...
//      }
      ...
      if (
        this.isDJTower() &&
        (property === "Damage" || property === "Cooldown")
      ) {
        return true;
      }
      ...
      if (
        this.isArcherTower() &&
        (property === "Hidden" || property === "Flying" || property === "Lead")
      ) {
        return true;
      }
    }
  ...
  createUnitButtons() {
    ...
    if (
      !this.viewer.activeUnits ||
      Object.keys(this.viewer.activeUnits).length === 0
    ) {
