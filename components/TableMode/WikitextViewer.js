class WikitextViewer {
  ...
  highlightWikitext(text) {
    ...
    text = text.replace(
      /(&lt;!--)([\s\S]*?)(&gt;)/g,
      '<span class="wiki-comment">$1$2$3</span>',
    );
    ...
    text = text.replace(
      /(\{\||\|\})/g,
      '<span class="table-delimiter">$1</span>',
    );
    text = text.replace(/(\|-)/g, '<span class="table-row">$1</span>');
    text = text.replace(
      /(\|\+\s*)('''[^']+''')/g,
      '<span class="table-caption">$1</span><span class="table-caption-text">$2</span>',
    );

    // cell and header delimiters
    text = text.replace(
      /(!!)/g,
      '<span class="table-header-delimiter">$1</span>',
    );
    text = text.replace(
      /(\|\|)/g,
      '<span class="table-cell-delimiter">$1</span>',
    );
    ...
    text = text.replace(
      /(''')(.*?)(''')/g,
      '<span class="wiki-bold">$1</span><span class="wiki-bold-text">$2</span><span class="wiki-bold">$3</span>',
    );
